...
namespace EventEaseApp.Controllers
{
    public class BookingsController : Controller
    {
        ...
        public ActionResult ManageBookings(string searchTerm, int? bookingId, int? eventTypeID, string dateFrom, string dateTo, string venueAvailableOnly)
        {
            // Parse dateFrom and dateTo into nullable DateTime
            ...
            // Parse venueAvailableOnly dropdown string ("true", "false", or null)
            bool? venueAvailableBool = null;
            if (!string.IsNullOrEmpty(venueAvailableOnly))
            {
                if (venueAvailableOnly.ToLower() == "true")
                    venueAvailableBool = true;
                else if (venueAvailableOnly.ToLower() == "false")
                    venueAvailableBool = false;
                // else leave null (no filter)
            }

            // Call stored procedure with correct params, converting bool? to int? (1 or NULL)
            var results = db.Database.SqlQuery<BookingSearchResultVM>(
                "EXEC sp_SearchBookings @SearchTerm, @BookingID, @EventName, @DateFrom, @DateTo, @EventTypeID, @VenueAvailableOnly",
                ...
                new SqlParameter("@EventName", DBNull.Value), // Assuming no filter on event name here
                new SqlParameter("@DateFrom", dtFrom.HasValue ? (object)dtFrom.Value : DBNull.Value),
                new SqlParameter("@DateTo", dtTo.HasValue ? (object)dtTo.Value : DBNull.Value),
                new SqlParameter("@EventTypeID", eventTypeID.HasValue ? (object)eventTypeID.Value : DBNull.Value),
                new SqlParameter("@VenueAvailableOnly", venueAvailableBool.HasValue ? (object)(venueAvailableBool.Value ? 1 : 0) : DBNull.Value)
            ).ToList();

            // Populate ViewBags for dropdowns, search terms etc.
            ViewBag.EventTypes = new SelectList(db.EventTypes.ToList(), "EventTypeID", "TypeName");
            ViewBag.VenueAvailableOnly = venueAvailableBool ?? false;
